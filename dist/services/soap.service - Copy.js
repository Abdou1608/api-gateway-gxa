...
async function sendSoapRequest(params, actionName, basSecurityContext, _sid, data) {
    ...
    if (!basSecurityContext) {
        ...
        const result = await runBasAct.RunAction(an, params, basSecurityContext ? basSecurityContext : new BasSecurityContext_1.BasSecurityContext(), xmldata).then(async (response) => {
            let e = new BasSoapFault_1.BasSoapFault();
            if (BasSoapFault_1.BasSoapFault.IsBasError(response)) {
                let f = BasSoapFault_1.BasSoapFault.ParseBasErrorDetailed(response);
                //throw new Error(JSON.stringify( f)
                throw new Error("Erreur BAS :" + f?.faultstring + "," + "BasFaultDetail:" + JSON.stringify(f));
                // throw new Error()  res.status(Number(f.faultcode) || 500).json({ error: f?.faultstring, detail: JSON.stringify(f) });
            }
            ;
