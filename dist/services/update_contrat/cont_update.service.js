...
async function cont_update(contrat, effet, piece, data, BasSecurityContext, ctx) {
...
async function cont_piece_update(contrat, produit, piece, Effet, data, BasSecurityContext, ctx) {
    const params = new BasParams_1.BasParams();
    params.AddStr("BasSecurityContext", BasSecurityContext.ToSoapVar());
    params.AddInt("contrat", contrat);
    if (piece) {
    ...
    const result = await (0, soap_service_1.sendSoapRequest)(params, "Cont_Updatepiece", BasSecurityContext, "piec", data, ctx);
    return result;
}
