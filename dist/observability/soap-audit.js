...
class SoapAuditRegistry {
    ...
    record(entry) {
        ...
        fs_1.promises.appendFile(LOG_FILE, line).catch(() => { });
    }
    snapshot(limit = 200) {
    ...
    async clear() {
        this.entries = [];
        try {
            await fs_1.promises.writeFile(LOG_FILE, '');
        }
        catch { /* ignore */ }
    }
    logPath() { return LOG_FILE; }
}
const INDEX_FILE = LOG_FILE + '.index.jsonl';
const INDEX_META = LOG_FILE + '.index.meta.json';
async function readJsonL(filePath, onLine, startOffset = 0) {
...
async function getIndexMeta() {
    try {
        const txt = await fs_1.promises.readFile(INDEX_META, 'utf8');
...
async function setIndexMeta(meta) { await fs_1.promises.writeFile(INDEX_META, JSON.stringify(meta)); }
async function appendToIndexFrom(offsetStart = 0) {
    ...
    await readJsonL(LOG_FILE, (obj, _raw, off) => {
...
async function rebuildIndex() {
    try {
        await fs_1.promises.unlink(INDEX_FILE);
    }
    catch { }
...
async function ensureIndexUpToDate() {
    ...
    const stat = await fs_1.promises.stat(LOG_FILE).catch(() => null);
    if (!stat)
        return;
...
async function queryFile(opts = {}) {
    ...
    let collected = [];
    let matchedCount = 0;
    const ringBuffer = []; // used for order=desc with no cursor
    const include = (e) => {
        if (actionQ && !(e.action || '').toLowerCase().includes(actionQ))
            return false;
        if (ownerQ && !(e.owner || '').toLowerCase().includes(ownerQ))
            return false;
        if (outcomeQ && e.outcome !== outcomeQ)
            return false;
        if (typeof since === 'number' && (e.start || 0) < since)
            return false;
        if (typeof until === 'number' && (e.start || 0) > until)
            return false;
        if (sortBy === 'start' && cursor) {
            // cursor applies on start ordering
            const cmp = (a, b) => (a.start - b.start) || ((a.id || 0) - (b.id || 0));
            if (order === 'desc') {
                if (!(cmp(e, cursor) < 0))
                    return false;
            }
    ...
    const earlyStop = (e) => {
        if (sortBy !== 'start')
            return false;
        if (typeof until === 'number' && (e.start || 0) > until)
            return true; // past time window (ascending file)
        return false;
    };
    const stat = await fs_1.promises.stat(LOG_FILE).catch(() => null);
    const size = stat?.size ?? 0;
    const processFromOffset = async (offset) => {
        collected = [];
        matchedCount = 0;
        ringBuffer.length = 0;
        await readJsonL(LOG_FILE, (obj) => {
            const e = obj;
            if (earlyStop(e))
                return false; // stop streaming early when beyond 'until'
            if (!include(e))
                return true;
            matchedCount++;
            if (sortBy === 'start') {
            ...
            if (sortBy === 'start' && order === 'desc' && cursor && collected.length >= limit)
                return false;
            return true;
        }, offset);
    };
...
exports.SoapAudit = Object.assign(new SoapAuditRegistry(), {
    queryFile,
    ensureIndexUpToDate,
    rebuildIndex,
});
