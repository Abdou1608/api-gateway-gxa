...
router.post('/', (0, zodValidator_1.validateBody)(api_detail_produitValidator_1.api_detail_produitValidator), async (req, res, next) => {
    try {
        ...
        const result = await (0, produit_details_service_1.produit_details)(code, _BasSecurityContext, options, basecouv, clauses);
        res.json(result);
    }
    catch (error) {
        return next(error);
