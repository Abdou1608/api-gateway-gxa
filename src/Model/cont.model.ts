export interface Cont {
  ...
  export const ContTagMap: Record<keyof Cont, string> = {
    contrat: 'N° contrat',
    numcont:'N° contrat',
    numtiers: 'N° tiers',
    frac: 'Fractionnement',
  ...
  export class Cont implements Cont {
  
  
    constructor(data?: Partial<Cont>) {
      if (data) {
    ...
    toJSON(): Partial<Cont> {
      const obj: Partial<Cont> = {};
      for (const key in this) {
        if (this.hasOwnProperty(key)) {
          obj[key as keyof Cont] = this[key];
        }
      }
      return obj;
    }
